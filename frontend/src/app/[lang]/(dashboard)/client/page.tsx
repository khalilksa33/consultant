...
export default function ClientDashboardPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  if (loading) {
    return <div className="p-8 text-brand-slate">Loading...</div>;
  }
      ...
      {projects.length === 0 ? (
        <div className="bg-white p-8 rounded shadow-sm text-center border border-brand-mist">
          <p className="text-brand-slate/80 text-lg">
            {lang === 'ar' ? 'لا يوجد مشاريع مسجلة حالياً.' : 'No projects registered currently.'}
          </p>
        </div>
      ) : (
